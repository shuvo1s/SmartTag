...
(the API connects to PostgreSQL through `@prisma/adapter-pg`) and `deepmerge-ts` merges Prisma configuration,
not request data.

## Variable data and expressions

Variable data (test records now, imports and integrations later) is untrusted content. It is data
only — it never executes as JavaScript, HTML, SVG script, SQL, shell or file/network access:

- **Expressions** are parsed into plain syntax trees and interpreted over a closed function set; no
  `eval`, `Function` or `RegExp` construction from user input, no access to globals, `process`,
  `require`, the network, files or the environment. Adversarial inputs (`process.env`, `require()`,
  `fetch()`, `eval()`, `Function()`, `constructor`, `__proto__`, `prototype`, …) fail with structured
  errors before evaluation. Size, nesting, arguments and produced text are bounded. See
  [expressions.md](expressions.md#security).
- **Validation patterns** compile to a linear-time automaton; ReDoS patterns such as `(a+)+$` run in
  linear time; back-references and look-around are refused.
- **Prototype pollution**: field keys `__proto__`, `constructor`, `prototype` and the reserved `__`
  namespace are refused in schemas; record keys are read with own-property checks, never merged, and
  reported (`FORBIDDEN_FIELD_KEY`); normalized records are frozen null-prototype objects. Tests send raw
  JSON with `__proto__` through the API and the validator.
- **Rendering**: resolved text is drawn with canvas text APIs and escaped by the SVG renderer; markup
  in data stays text. URLs must be http(s) without credentials; `javascript:`/`data:` are refused.
- **Images**: data can only reference asset ids of the organization; remote URLs are never fetched
  into artwork. The validation API checks ids against the organization's placeable images and the
  designer loads them through tenant-scoped endpoints (cross-tenant ids are unavailable).
- **Limits**: ≤ 1 000 keys per record, 10 000 characters per text value, 2 048 per URL, 256 KB per
