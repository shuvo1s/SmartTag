...
function useCreateCustomer() {
  ...
  return useMutation({
    mutationFn: (input: CreateCustomerRequest) =>
      apiRequest<CustomerDto>('/customers', { json: input }),
...
function useCreateBrand(customerId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: CreateBrandRequest) =>
      apiRequest<BrandDto>(`/customers/${customerId}/brands`, { json: input }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['customers'] }),
  });
}
  ...
  return (
    <div className="max-w-5xl">
      <PageHeader
        ...
        title="Customers & brands"
        description="Templates can optionally belong to a customer and one of its brands."
      />
      {canManage ? (
        <Card className="mb-6">
              ...
              <CardHeader
                title={
                  <span className="inline-flex items-center gap-2">
                    {customer.name}{' '}
                    <code className="font-mono text-xs text-slate-500">{customer.code}</code>
                  </span>
                }
              />
              <CardBody className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {customer.brands.length === 0 ? (
                    <span className="text-sm text-slate-500">No brands</span>
                  ) : null}
