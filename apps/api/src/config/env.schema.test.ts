...
describe('loadAppConfig', () => {
  ...
  it('uses Secure __Host- cookies in production by default', () => {
    ...
    expect(config.auth).toMatchObject({
      cookieSecure: true,
      cookieName: '__Host-smarttag_session',
    });
  });

  it('refuses insecure cookies in production', () => {
    expect(() =>
      loadAppConfig({ ...base, NODE_ENV: 'production', AUTH_COOKIE_SECURE: 'false' }),
    ).toThrow(EnvironmentValidationError);
  });

  it('requires a bucket for S3 storage', () => {
    expect(() => loadAppConfig({ ...base, OBJECT_STORAGE_DRIVER: 's3' })).toThrow(
  ...
  it('parses and normalises allowed origins', () => {
    const config = loadAppConfig({
      ...base,
      API_ALLOWED_ORIGINS: 'https://tags.example.com/, http://localhost:3000',
    });
