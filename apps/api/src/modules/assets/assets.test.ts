...
import { detectMimeType, inspectContent, isMimeAllowedForAssetType, sanitizeFilename } from './asset-content-inspector';
import { LocalFilesystemStorage } from './storage/local-filesystem.storage';
import { ObjectNotFoundError, assertValidObjectKey, assetStorageKey } from './storage/object-storage';
...
describe('asset content inspection', () => {
  ...
  it('reads pixel dimensions for raster images', () => {
    ...
    expect(inspectContent(Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="600" height="240"/>'))).toEqual({
...
describe('object storage keys', () => {
  it('builds content-addressed, tenant-prefixed keys', () => {
    ...
    expect(assetStorageKey('org-1', checksum)).toBe(`organizations/org-1/assets/sha256/ab/${checksum}`);
  });

  it.each(['../secret', '/absolute', 'a//b', 'trailing/', 'spaces are bad', ''])('rejects unsafe key %p', (key) => {
...
describe('LocalFilesystemStorage', () => {
  ...
  it('stores, reads, checks and deletes objects', async () => {
    ...
    await storage.putObject(key, Buffer.from('hello'), { contentType: 'text/plain', checksumSha256: 'x' });
  ...
  it('refuses keys that would escape the storage root', async () => {
    await expect(storage.putObject('../escape', Buffer.from('x'), { contentType: 'text/plain', checksumSha256: 'x' })).rejects.toThrow();
  });
});
