...
function contextFor(handler: keyof ExampleController, request: Record<string, unknown>): ExecutionContext {
  return {
...
describe('PermissionsGuard', () => {
  ...
  it('requires authentication for everything else', () => {
    expect(() => guard.canActivate(contextFor('anyUser', {}))).toThrow(expect.objectContaining({ code: 'UNAUTHENTICATED' }));
  });
  ...
  it('denies handlers without any authorization policy (secure by default)', () => {
    expect(() => guard.canActivate(contextFor('undecorated', { actor: actorWith(['ORG_ADMIN']) }))).toThrow(
      expect.objectContaining({ code: 'FORBIDDEN' }),
    );
  });
});
...
describe('OriginGuard', () => {
  const guard = new OriginGuard(loadAppConfig({ DATABASE_URL: 'postgresql://x@h/db', API_ALLOWED_ORIGINS: 'http://localhost:3000' }));
  const run = (method: string, headers: Record<string, string>) => guard.canActivate(contextFor('create', { method, headers }));
  ...
  it('rejects cross-origin state-changing requests', () => {
    expect(() => run('POST', { origin: 'https://evil.example' })).toThrow(expect.objectContaining({ code: 'FORBIDDEN' }));
    expect(() => run('DELETE', { 'sec-fetch-site': 'cross-site' })).toThrow(expect.objectContaining({ code: 'FORBIDDEN' }));
    expect(() => run('POST', { origin: 'null' })).toThrow(expect.objectContaining({ code: 'FORBIDDEN' }));
  });
});
