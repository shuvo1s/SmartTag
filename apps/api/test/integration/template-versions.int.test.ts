...
describe('template versions: creation, drafts, lifecycle and immutability', () => {
  ...
  const sampleFor = (templateId: string): DesignDocument => createSampleHangTagDocument({ documentId: templateId });

  async function createVersion(document: unknown = sampleFor(template.id), agent = designer) {
    return agent.post(`${API}/templates/${template.id}/versions`).send({ document, changeSummary: 'Artwork' });
  }
  ...
  it('creates the next version from a full document and moves the head pointer', async () => {
    ...
    expect(version.summary.boundFieldKeys).toEqual(expect.arrayContaining(['product_name', 'size', 'price', 'gtin']));

    const refreshed = (await designer.get(`${API}/templates/${template.id}`)).body as TemplateDto;
    expect(refreshed).toMatchObject({ latestVersionNumber: 2, currentVersion: { id: version.id, versionNumber: 2 } });
  ...
  it('copies content from a base version when no document is supplied', async () => {
    ...
    const v3 = await designer.post(`${API}/templates/${template.id}/versions`).send({ basedOnVersionId: v2.id });
    expect(v3.status).toBe(201);
    expect(v3.body).toMatchObject({ versionNumber: 3, basedOnVersionId: v2.id, documentHash: v2.documentHash });
  });
  ...
  it('allocates unique, gap-free version numbers under concurrency', async () => {
    ...
    const numbers = responses.map((r) => (r.body as TemplateVersionDetailDto).versionNumber).sort((a, b) => a - b);
    expect(numbers).toEqual([2, 3, 4, 5, 6, 7]);
  });

  it.each([
    ['structurally invalid', { schemaVersion: 1, pages: [] }, 'INVALID_DOCUMENT', 422],
    ['future schema version', { ...createSampleHangTagDocument(), schemaVersion: 99 }, 'UNSUPPORTED_SCHEMA_VERSION', 422],
  ...
  it('rejects documents that belong to another template or reference missing assets', async () => {
    ...
    expect(foreign.body.error.details.documentIssues.map((i: { code: string }) => i.code)).toContain('DOCUMENT_ID_MISMATCH');
  ...
  describe('draft editing', () => {
    it('updates a draft with optimistic concurrency and a new hash', async () => {
      ...
      edited.pages[1]!.objects.push(createTextObject({ id: 'care', x: 20, y: 20, width: 80, height: 10, zIndex: 50, content: 'Wash cold' }));

      const response = await designer.patch(`${API}/template-versions/${v2.id}`).send({ document: edited, expectedRevision: 1 });
      expect(response.status).toBe(200);
      expect(response.body).toMatchObject({ revision: 2, documentHash: await computeDocumentHash(edited) });
      expect(response.body.documentHash).not.toBe(v2.documentHash);

      const stale = await designer.patch(`${API}/template-versions/${v2.id}`).send({ document: edited, expectedRevision: 1 });
      expect(stale.status).toBe(409);
      expect(stale.body.error).toMatchObject({ code: 'VERSION_CONFLICT', details: { currentRevision: 2 } });
      expect(await t.prisma.auditEvent.count({ where: { action: 'TEMPLATE_VERSION_UPDATED' } })).toBe(1);
    });

    it('requires template-version:edit-draft', async () => {
      const viewer = await loginAs(t, 'viewer.a@test.local');
      const v2 = (await createVersion()).body as TemplateVersionDetailDto;
      const response = await viewer.patch(`${API}/template-versions/${v2.id}`).send({ document: sampleFor(template.id), expectedRevision: 1 });
  ...
  describe('status lifecycle', () => {
    it('runs DRAFT → IN_REVIEW → APPROVED → RETIRED with the right roles', async () => {
      ...
      expect(approved.body).toMatchObject({ status: 'APPROVED', approvedBy: { id: tenants.users.approverA.id } });
      ...
      const audit = await t.prisma.auditEvent.findMany({ where: { action: 'TEMPLATE_VERSION_STATUS_CHANGED' }, orderBy: { occurredAt: 'asc' } });
    ...
    it('never modifies an approved version: edits are refused and approval cannot be undone', async () => {
      ...
      const inReviewEdit = await designer.patch(`${API}/template-versions/${v2.id}`).send({ document: sampleFor(template.id), expectedRevision: 1 });
      ...
      const approvedEdit = await admin.patch(`${API}/template-versions/${v2.id}`).send({ document: sampleFor(template.id), expectedRevision: 1 });
      ...
      expect(stored).toMatchObject({ status: 'APPROVED', revision: 1, documentHash: v2.documentHash });

      // Editing means creating a new version based on the approved one.
      const v3 = await designer.post(`${API}/templates/${template.id}/versions`).send({ basedOnVersionId: v2.id, changeSummary: 'Next revision' });
      expect(v3.status).toBe(201);
    ...
    it('refuses approval when the stored document no longer matches its hash', async () => {
      const v2 = (await createVersion()).body as TemplateVersionDetailDto;
      await t.prisma.templateVersion.update({ where: { id: v2.id }, data: { documentHash: 'f'.repeat(64) } });
      const response = await transition(designer, v2.id, 'IN_REVIEW');
      expect(response.status).toBe(500);
      expect(response.body.error).toMatchObject({ code: 'INTERNAL_ERROR', message: 'Document integrity check failed' });
    });
