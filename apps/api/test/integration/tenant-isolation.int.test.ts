...
describe('tenant isolation', () => {
  ...
  beforeEach(async () => {
    ...
    const customer = await adminA.post(`${API}/customers`).send({ code: 'A-CUST', name: 'A customer' });
    const brand = await adminA.post(`${API}/customers/${customer.body.id}/brands`).send({ code: 'A-BRAND', name: 'A brand' });
    customerA = { id: customer.body.id, brandId: brand.body.id };
    templateA = (await adminA.post(`${API}/templates`).send(hangTagRequest({ code: 'A-SECRET', customerId: customerA.id }))).body;
    assetA = (await adminA.post(`${API}/assets`).field('assetType', 'IMAGE').attach('file', TINY_PNG, 'a.png')).body.id;
  });
  ...
  it.each([
    ...
    ['PATCH template', (agent: TestAgent) => agent.patch(`${API}/templates/${templateA.id}`).send({ name: 'pwned' })],
    ['GET versions', (agent: TestAgent) => agent.get(`${API}/templates/${templateA.id}/versions`)],
    ['POST version', (agent: TestAgent) => agent.post(`${API}/templates/${templateA.id}/versions`).send({})],
    ['GET version', (agent: TestAgent) => agent.get(`${API}/template-versions/${templateA.currentVersion!.id}`)],
    [
      'PATCH version',
      (agent: TestAgent) =>
        agent.patch(`${API}/template-versions/${templateA.currentVersion!.id}`).send({ document: {}, expectedRevision: 1 }),
    ],
    [
      'transition version',
      (agent: TestAgent) => agent.post(`${API}/template-versions/${templateA.currentVersion!.id}/transitions`).send({ targetStatus: 'IN_REVIEW' }),
    ],
    ['GET customer', (agent: TestAgent) => agent.get(`${API}/customers/${customerA.id}`)],
    ['POST brand on customer', (agent: TestAgent) => agent.post(`${API}/customers/${customerA.id}/brands`).send({ code: 'X', name: 'X' })],
  ...
  it('leaves the other tenant\'s data untouched', async () => {
    ...
    await adminB.post(`${API}/template-versions/${templateA.currentVersion!.id}/transitions`).send({ targetStatus: 'IN_REVIEW' });
    const stored = await t.prisma.template.findUniqueOrThrow({ where: { id: templateA.id }, include: { currentVersion: true } });
  ...
  it('refuses to link another tenant\'s customer or brand', async () => {
    const response = await adminB.post(`${API}/templates`).send(hangTagRequest({ code: 'B-STEAL', customerId: customerA.id, brandId: customerA.brandId }));
    expect(response.status).toBe(400);
    expect(response.body.error.details.fieldErrors).toEqual([{ path: 'customerId', message: 'Customer not found' }]);
  });

  it('refuses documents that reference another tenant\'s assets', async () => {
    const templateB = (await adminB.post(`${API}/templates`).send(hangTagRequest({ code: 'B-TAG' }))).body as TemplateDto;
    const document = createSampleHangTagDocument({ documentId: templateB.id, logoAssetId: assetA });
    const response = await adminB.post(`${API}/templates/${templateB.id}/versions`).send({ document });
  ...
  it('a user in both tenants sees only the active organization', async () => {
    ...
    expect((await multi.get(`${API}/templates`)).body.items.map((i: TemplateDto) => i.code)).toEqual(expectedCodes);
  ...
  it('service layer scopes by the actor organization, independent of HTTP', async () => {
    ...
    await expect(service.update(actorB, templateA.id, { name: 'x' })).rejects.toMatchObject({ code: 'NOT_FOUND' });
  ...
  it('the database rejects cross-tenant references even if application code were bypassed', async () => {
    ...
    await expect(
      t.prisma.$executeRawUnsafe(`UPDATE templates SET customer_id = $1::uuid WHERE id = $2::uuid`, customerA.id, templateB.body.id),
    ).rejects.toThrow(/foreign key|violates/i);
    await expect(
      t.prisma.$executeRawUnsafe(`UPDATE templates SET organization_id = $1::uuid WHERE id = $2::uuid`, tenants.orgB.id, templateA.id),
