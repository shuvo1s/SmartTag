...
import { isArtworkObjectType } from '../objects';
 ...
 *   2. object types  — unknown artwork types are reported precisely instead of as union noise
...
export function validateDesignDocument(input: unknown): DocumentValidationResult {
  ...
  const unsupportedObjectPaths = reportUnsupportedObjectTypes(input, issues);
  ...
  if (!parsed.success) {
    for (const zodIssue of parsed.error.issues) {
      ...
      if (unsupportedObjectPaths.some((prefix) => startsWith(path, prefix))) {
        continue; // already reported as UNSUPPORTED_OBJECT_TYPE
      }
