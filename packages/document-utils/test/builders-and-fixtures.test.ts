...
describe('createBlankDesignDocument', () => {
  it('creates a valid front-only document with geometry converted to points', () => {
    const document = createBlankDesignDocument({
      name: 'Care label', documentType: 'CARE_LABEL', unit: 'in', width: 1.5, height: 3, bleed: 0.125, safeMargin: 0.0625, pageLayout: 'FRONT_ONLY',
    });
    expect(validateDesignDocument(document)).toMatchObject({ valid: true, errors: [], warnings: [] });
    expect(document.dimensions).toMatchObject({ width: 108, height: 216, orientation: 'PORTRAIT', displayUnit: 'in' });
  ...
  it('creates front and back pages and derives landscape orientation', () => {
    const document = createBlankDesignDocument({
      name: 'Ticket', documentType: 'PRICE_TICKET', unit: 'mm', width: 60, height: 40, bleed: 2, safeMargin: 2, pageLayout: 'FRONT_AND_BACK',
    });
    expect(validateDesignDocument(document).valid).toBe(true);
    expect(document.dimensions.orientation).toBe('LANDSCAPE');
    expect(document.pages.map((page) => [page.id, page.side])).toEqual([['page-front', 'FRONT'], ['page-back', 'BACK']]);
  });

  it('assigns a fresh document id unless one is given', () => {
    const options = { name: 'x', documentType: 'HANG_TAG', unit: 'mm', width: 10, height: 10, bleed: 0, safeMargin: 1, pageLayout: 'FRONT_ONLY' } as const;
    expect(createBlankDesignDocument(options).documentId).not.toBe(createBlankDesignDocument(options).documentId);
    expect(createBlankDesignDocument({ ...options, documentId: SAMPLE_HANG_TAG_DOCUMENT_ID }).documentId).toBe(SAMPLE_HANG_TAG_DOCUMENT_ID);
  });
});
...
describe('sample hang tag fixture (50 × 90 mm, 3 mm bleed, 3 mm safe, front + back)', () => {
  ...
  it('summarises the document', () => {
    ...
    expect(summary).toMatchObject({ documentType: 'HANG_TAG', pageCount: 2, pageSides: ['FRONT', 'BACK'], dataFieldCount: 10 });
    expect(summary.boundFieldKeys).toEqual(expect.arrayContaining(['product_name', 'size', 'price', 'gtin']));
...
describe('asset references and effective resolution', () => {
  it('collects image asset references including image-field defaults', () => {
    ...
    expect(collectAssetReferences(document)).toEqual([SAMPLE_BRAND_LOGO_ASSET_ID, defaultImage].sort());
  });
  ...
  it('accounts for fit mode and crop', () => {
    ...
    expect(computeEffectiveResolution(source, { ...frame, fitMode: 'CONTAIN', crop: null }).effectivePpi).toBeCloseTo(2000, 6);
    // COVER: 1000 px spans the 1 in frame height → 1000 ppi
    expect(computeEffectiveResolution(source, { ...frame, fitMode: 'COVER', crop: null }).effectivePpi).toBeCloseTo(1000, 6);
    // Cropping to the left half leaves 1000×1000 px on a 1 in square → 1000 ppi
    expect(
      computeEffectiveResolution(source, { ...frame, fitMode: 'CONTAIN', crop: { x: 0, y: 0, width: 0.5, height: 1 } }).effectivePpi,
    ).toBeCloseTo(1000, 6);
  });
});
