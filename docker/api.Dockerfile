# SmartTag API (@smarttag/api, NestJS) — production image.
#
#   docker build -f docker/api.Dockerfile -t smarttag-api .
#
# Build context: the repository root (the API is one workspace of an npm workspaces monorepo and
# builds against the other workspaces).
#
# The image contains no secrets and no .env file: DATABASE_URL, REDIS_URL, the object storage
# settings, the auth settings and the IMPORT_*/PRODUCTION_* limits are all read from the process
# environment at startup (apps/api/src/config/env.schema.ts). Database migrations are NEVER run by
# this image — they are a separate, controlled deployment step (docs/dokploy-deployment.md).

ARG NODE_IMAGE=node:24-bookworm-slim

# ------------------------------------------------------------------------------------------------
# base — the runtime Node.js, shared by every stage
# ------------------------------------------------------------------------------------------------
FROM ${NODE_IMAGE} AS base
ENV NEXT_TELEMETRY_DISABLED=1 \
    TURBO_TELEMETRY_DISABLED=1 \
    DO_NOT_TRACK=1
WORKDIR /app

# ------------------------------------------------------------------------------------------------
# manifests — every workspace manifest plus the lockfile, so `npm ci` caches independently of source
# ------------------------------------------------------------------------------------------------
FROM base AS manifests
COPY package.json package-lock.json .npmrc turbo.json tsconfig.base.json ./
COPY packages/barcode-bwip/package.json packages/barcode-bwip/
COPY packages/barcode-core/package.json packages/barcode-core/
COPY packages/canvas-adapter/package.json packages/canvas-adapter/
COPY packages/config/package.json packages/config/
COPY packages/data-core/package.json packages/data-core/
COPY packages/database/package.json packages/database/
COPY packages/document-schema/package.json packages/document-schema/
COPY packages/document-utils/package.json packages/document-utils/
COPY packages/editor-core/package.json packages/editor-core/
COPY packages/expression-core/package.json packages/expression-core/
COPY packages/import-core/package.json packages/import-core/
COPY packages/import-processing/package.json packages/import-processing/
COPY packages/object-storage/package.json packages/object-storage/
COPY packages/production-core/package.json packages/production-core/
COPY packages/production-processing/package.json packages/production-processing/
COPY packages/rendering-core/package.json packages/rendering-core/
COPY packages/shared-types/package.json packages/shared-types/
COPY packages/tabular-sources/package.json packages/tabular-sources/
COPY packages/ui/package.json packages/ui/
COPY apps/api/package.json apps/api/
COPY apps/web/package.json apps/web/
COPY apps/worker/package.json apps/worker/
COPY e2e/package.json e2e/

# ------------------------------------------------------------------------------------------------
# build — install every dependency and compile the API with its workspace dependencies
# ------------------------------------------------------------------------------------------------
FROM manifests AS build
# Prisma's CLI detects the platform's OpenSSL when it generates the client.
RUN apt-get update \
  && apt-get install -y --no-install-recommends openssl \
  && rm -rf /var/lib/apt/lists/*
RUN npm ci --no-audit --no-fund
COPY . .
# `build` depends on `^build`, so this compiles every workspace the API needs — including
# @smarttag/database, whose build runs `prisma generate` (no database connection required).
RUN npx turbo run build --filter=@smarttag/api

# ------------------------------------------------------------------------------------------------
# migrate — a one-off image for the controlled migration step, NOT part of the running service
# ------------------------------------------------------------------------------------------------
#   docker build -f docker/api.Dockerfile --target migrate -t smarttag-migrate .
#   docker run --rm -e DATABASE_URL=… smarttag-migrate
#
# It exists because the runtime image below deliberately has no Prisma CLI: migrations are applied
# by an operator, once per release, never by a starting container. `prisma migrate deploy` only
# applies committed migrations — it never resets, never pushes and never seeds.
FROM build AS migrate
WORKDIR /app
CMD ["npm", "run", "db:migrate:deploy"]

# ------------------------------------------------------------------------------------------------
# prune — the same tree with development dependencies removed
# ------------------------------------------------------------------------------------------------
FROM build AS prune
# Development dependencies (TypeScript, the Prisma CLI, test tooling) do not ship.
RUN npm prune --omit=dev --no-audit --no-fund
# npm keeps a few dependencies next to the workspace that needs them rather than hoisting them
# (apps/api/node_modules/nestjs-pino, for one). The directory is created unconditionally so the
# runner's COPY works whether or not anything was nested.
RUN mkdir -p apps/api/node_modules

# ------------------------------------------------------------------------------------------------
# runner — compiled JavaScript, production dependencies, non-root
# ------------------------------------------------------------------------------------------------
FROM base AS runner
ENV NODE_ENV=production \
    API_HOST=0.0.0.0 \
    API_PORT=4000
COPY --from=prune --chown=node:node /app/package.json /app/package-lock.json ./
COPY --from=prune --chown=node:node /app/node_modules ./node_modules
# Workspace packages: node_modules/@smarttag/* are symlinks into these directories.
COPY --from=prune --chown=node:node /app/packages ./packages
COPY --from=prune --chown=node:node /app/apps/api/package.json ./apps/api/package.json
COPY --from=prune --chown=node:node /app/apps/api/node_modules ./apps/api/node_modules
COPY --from=prune --chown=node:node /app/apps/api/dist ./apps/api/dist

USER node
WORKDIR /app/apps/api
EXPOSE 4000

# The health endpoint is public and checks the database (apps/api/src/modules/health).
HEALTHCHECK --interval=30s --timeout=5s --start-period=30s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:'+(process.env.API_PORT||4000)+'/api/v1/health').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"

# Equivalent to `npm run start -w @smarttag/api`, without the npm process in between.
CMD ["node", "dist/main.js"]
